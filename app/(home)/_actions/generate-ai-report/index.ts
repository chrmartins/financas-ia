...
export const generateAiReport = async (month: string) => {
  ...
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar minha vida financeira. Cite cempre no final do relatorio uma pequena frase positiva em portugues de algum escritor ou pessoa renomada do mercado financeiro mundial. As transações estão divididas por ponto e vírgula. A estrutura de cada uma é {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. São elas:
        ${transactions
          .map(
            (transaction) =>
